...
import { ArrowRight, Edit2, Plus, Trash2, FileText, Star, MapPin } from "lucide-react";
  ...
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="overflow-hidden border border-neutral-200 bg-white flex flex-col animate-pulse">
  ...
  return (
    <motion.div whileHover={{ y: -5 }} transition={{ duration: 0.35 }}
      ...
      {blog.image || blog.images?.[0] ? (
        <img src={blog.images?.[0] || blog.image} alt={blog.title}
      ...
      {/* Admin */}
      {isAdmin && (
        <button onClick={e => { e.stopPropagation(); onEdit(blog); }}
      ...
      <div className="absolute bottom-0 left-0 right-0 px-6 pb-7 pt-12 max-w-[640px]">
        ...
        <p className="font-body text-white/60 text-[13px] leading-relaxed line-clamp-2 mb-4">{blog.description}</p>
        <motion.div className="flex items-center gap-1.5 font-heading font-bold text-[11px] uppercase text-white/80 group-hover:text-primary-400 transition-colors"
          whileHover={{ x: 4 }}>
...
function FeaturedSideCard({ blog, isAdmin, onEdit, navigate, delay = 0 }) {
  ...
  return (
    <FadeUp delay={delay}>
      <motion.div whileHover={{ y: -4, boxShadow: "0 14px 36px rgba(0,0,0,0.1)" }}
        ...
        <div className="flex flex-col justify-between p-4 flex-1 min-w-0">
          <div>
            {blog.category && (
              <span className="font-heading font-bold text-[9px] tracking-[0.12em] uppercase text-primary-600">{blog.category}</span>
            )}
          ...
          <div className="flex items-center justify-between mt-2">
            <span className="font-body text-[10px] text-neutral-400">{blog.date}</span>
...
function BlogCard({ blog, isAdmin, onEdit, onDelete, navigate }) {
  ...
  return (
    <motion.div
      ...
      {/* Image */}
      <div className="relative h-48 overflow-hidden flex-shrink-0 cursor-pointer"
        onClick={() => navigate(`/blog/${blog.id}`)}>
        {cover ? (
          <motion.img src={cover} alt={blog.title} loading="lazy"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-primary-50">
            <FileText size={28} className="text-primary-400 opacity-40" />
            <p className="text-[11px] font-heading font-semibold tracking-widest uppercase text-primary-400 opacity-50">No Cover</p>
          </div>
        )}
      ...
      <div className="p-5 flex-1 flex flex-col">
        <h3 className="font-heading font-bold text-[16px] leading-snug line-clamp-2 text-secondary-600 hover:text-primary-600 transition-colors cursor-pointer"
          onClick={() => navigate(`/blog/${blog.id}`)}>
          {blog.title}
        </h3>
        ...
        <div className="mt-4 pt-3.5 flex items-center justify-between text-[11px] border-t border-neutral-200">
          ...
          <span className="flex items-center gap-1.5">
            ...
            <span className="font-heading font-bold tracking-[0.14em] uppercase text-[10px] text-neutral-400">Real Estate</span>
          </span>
        </div>

        {/* Actions */}
        <div className={`mt-4 grid gap-2 ${isAdmin ? "grid-cols-3" : "grid-cols-1"}`}>
          {isAdmin && (
            <>
              ...
              <AnimatePresence mode="wait">
                {confirmDelete ? (
                  <motion.button key="confirm" initial={{ scale: 0.95 }} animate={{ scale: 1 }}
                    exit={{ scale: 0.95 }} whileTap={{ scale: 0.97 }}
                  ...
                  <motion.button key="delete" initial={{ scale: 0.95 }} animate={{ scale: 1 }}
                    exit={{ scale: 0.95 }} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20">
      <div className="max-w-md mx-auto border border-neutral-200 bg-white p-12">
        ...
        <h3 className="font-heading font-bold text-[18px] text-secondary-600">No Articles Yet</h3>
...
export default function BlogSection() {
  ...
  const handleSave = async (blogData) => {
    if (editingBlog) { await updateBlog(editingBlog.id, blogData); setEditingBlog(null); }
    else             { await createBlog(blogData);                  setIsCreating(false); }
  };

  if (editingBlog || isCreating) {
    return (
      <BlogForm editingBlog={editingBlog} onSave={handleSave}
  ...
  return (
    <section id="blog" className="relative py-20 md:py-28 px-[5%] bg-neutral-100">
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                <div className="inline-flex items-center gap-2 mb-3">
                  ...
                  <p className="text-[11px] tracking-[0.2em] uppercase font-heading font-bold text-primary-600">From the Desk</p>
                </div>
                <h2 className="font-heading leading-[1.15] text-secondary-600"
                  style={{ fontSize: "clamp(2rem, 3.5vw, 2.8rem)", fontWeight: 400 }}>
                  Property{" "}
                  <em className="not-italic text-primary-600 font-semibold">Insights</em>{" "}
              ...
              <div className="md:max-w-[340px] flex flex-col items-start md:items-end gap-4">
                <p className="font-body text-sm leading-relaxed md:text-right text-neutral-500">
                  Expert articles on Ibadan's property market, investment opportunities, and real estate advice.
                </p>
                {isAdmin && (
                  <motion.button whileHover={{ scale: 1.03, y: -2 }} whileTap={{ scale: 0.97 }}
              ...
              <FadeUp>
                <div className="mb-14">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4">
                    <FeaturedHeroCard blog={featuredHero} isAdmin={isAdmin} onEdit={setEditingBlog} navigate={navigate} />
                    <div className="flex flex-col gap-4">
                      {featuredSide.map((b, i) => (
                        <FeaturedSideCard key={b.id} blog={b} isAdmin={isAdmin} onEdit={setEditingBlog} navigate={navigate} delay={0.08 + i * 0.06} />
                      ))}
                      {/* CTA block */}
                      <FadeUp delay={0.2}>
              ...
              {preview.map((blog, i) => (
                <FadeUp key={blog.id} delay={i * 0.07}>
                  <BlogCard blog={blog} isAdmin={isAdmin}
                    onEdit={setEditingBlog} onDelete={deleteBlog} navigate={navigate} />
                </FadeUp>
              ))}
            </div>
