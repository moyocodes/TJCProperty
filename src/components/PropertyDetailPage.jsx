...
import {
  ArrowLeft, MapPin, Home, Building2, Tag, Users,
  CheckCircle, Share2, Heart, ChevronLeft, ChevronRight,
  X, MessageCircle, PhoneCall, Mail, ExternalLink,
  Calendar, Sparkles, Link2, Check,
} from "lucide-react";
...
function ShareSheet({ title }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8, scale: 0.97 }}
      animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.97 }}
      ...
      {[
        {
          label: copied ? "Copied!" : "Copy Link", icon: copied ? <Check size={13} /> : <Link2 size={13} />,
        ...
        {
          label: "Twitter / X", icon: <Share2 size={13} />,
...
function RelatedCard({ listing, onView }) {
  ...
  return (
    <motion.div whileHover={{ y: -4, boxShadow: "0 12px 32px rgba(0,0,0,0.1)" }}
      ...
      <div className="p-3">
        <h4 className="font-heading font-bold text-[12px] text-secondary-600 line-clamp-1 group-hover:text-primary-600 transition-colors">{listing.name}</h4>
    ...
    return (
      <div className="min-h-screen bg-neutral-100 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="font-body text-sm text-neutral-500 mb-6">This listing may have been removed or the link is incorrect.</p>
        ...
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
              ...
              className="hidden sm:flex items-center gap-1.5 h-9 px-5 bg-primary-600 hover:bg-primary-500 text-white font-heading font-bold text-[11px] uppercase border-none cursor-pointer transition-colors">
              Enquire Now
            </motion.button>
          </div>
        </div>
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
              <div className="flex flex-wrap items-center gap-2 mb-3">
                {listing.category && (
                  <span className="text-[10px] tracking-[0.18em] uppercase font-heading font-bold px-3 py-1 text-white bg-primary-600">{listing.category}</span>
                )}
                {listing.status && (
                  <span className={`text-[10px] tracking-[0.18em] uppercase font-heading font-bold px-3 py-1 ${statusColor}`}>{listing.status}</span>
                )}
              </div>
            ...
            <FadeUp delay={0.1} className="mt-10">
              <div className="bg-white border border-neutral-200 p-6">
                ...
                <div className="grid sm:grid-cols-3 gap-5">
                  {[
                    { icon: <MapPin size={18} />,      title: "Prime Location",  body: "Strategically located with easy access to major roads and city amenities." },
                    { icon: <Building2 size={18} />,   title: "Quality Finish",  body: "Built to high standards with durable materials and modern design." },
                    { icon: <CheckCircle size={18} />, title: "Verified Listing", body: "All documents verified by the TJC Properties team for your peace of mind." },
                  ].map((item, i) => (
          ...
          <aside>
            <div className="sticky top-20 space-y-4">
              <div className="bg-white border border-neutral-200 p-5">
                {listing.priceLabel && (
                  <>
                    <p className="font-heading font-bold text-[10px] tracking-[0.14em] uppercase text-neutral-400">Asking Price</p>
                    <p className="font-heading font-bold text-secondary-600 mt-1" style={{ fontSize: "clamp(1.3rem, 2.5vw, 1.8rem)" }}>
                      {listing.priceLabel}
                    </p>
                ...
                <p className="font-body text-[10px] text-neutral-400 text-center mt-3">Response within 2–4 hours on business days</p>
              </div>

              {/* Quick facts */}
              <div className="bg-white border border-neutral-200 p-5 space-y-3">
                <p className="font-heading font-bold text-[10px] tracking-[0.16em] uppercase text-neutral-400">Property Info</p>
                {[
                  { label: "Type",     value: listing.type?.[0]?.toUpperCase() + listing.type?.slice(1) || "—" },
                  { label: "Category", value: listing.category || "—" },
                  { label: "Units",    value: listing.units ?? "—" },
                  { label: "Status",   value: listing.status?.[0]?.toUpperCase() + listing.status?.slice(1) || "Available" },
                  { label: "Photos",   value: images.length },
                ].map((f, i) => (
                  <div key={i} className="flex items-center justify-between">
                    <span className="font-body text-[12px] text-neutral-500">{f.label}</span>
              ...
              <div className="bg-secondary-600 p-5">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 bg-primary-600 flex items-center justify-center font-heading font-bold text-white text-sm flex-shrink-0">TJC</div>
                  <div>
                    <p className="font-heading font-bold text-[13px] text-white">TJC Properties</p>
                    <p className="font-body text-[11px] text-white/55">Licensed Real Estate Agent</p>
                  </div>
                </div>
          ...
          <div className="relative overflow-hidden bg-secondary-600 p-8 sm:p-12">
            ...
            <div className="relative">
              ...
              <p className="font-body text-white/55 text-[14px] mt-2 max-w-[420px]">
                Our agents are available Mon–Sat, 8am to 6pm. Get in touch and we'll help you make the right move.
              </p>
