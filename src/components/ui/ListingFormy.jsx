...
import { X, Upload, ImagePlus, Loader2, Save, ArrowLeft, Plus, CheckCircle } from "lucide-react";
...
const CATS     = [
  "3 Bedroom Flat","2 Bedroom Flat","1 Bedroom Flat","Room & Parlour",
  "Self-Contain","Bungalow","Duplex","Warehouse","Office Space",
  "Retail Shop","Land","Other",
];
...
const base = (f, e) => ({
  width: "100%", background: T.white, fontFamily: "inherit", fontSize: 14, outline: "none",
  padding: "0.7rem 1rem", color: T.navy,
  border: `1px solid ${e ? T.danger : f ? T.primary : T.border}`,
...
function TInput({ value, onChange, placeholder, type = "text", error, disabled, ...rest }) {
  ...
  return <input type={type} value={value} onChange={onChange} placeholder={placeholder}
    ...
    style={{ ...base(f, error), opacity: disabled ? 0.6 : 1 }} {...rest} />;
}
...
function TTextarea({ value, onChange, placeholder, rows = 4, disabled }) {
  ...
  return <textarea value={value} onChange={onChange} placeholder={placeholder} rows={rows}
    disabled={disabled} onFocus={() => setF(true)} onBlur={() => setF(false)}
    style={{ ...base(f), resize: "vertical", opacity: disabled ? 0.6 : 1 }} />;
}
...
function TSelect({ value, onChange, options, placeholder, disabled, error }) {
  ...
  return (
    <select value={value} onChange={onChange} disabled={disabled}
      ...
      style={{
        ...base(f, error), cursor: "pointer", appearance: "none",
...
export default function ListingForm({ editingListing, onDone, onCancel }) {
  ...
  useEffect(() => {
    if (editingListing) {
      ...
      const imgs = editingListing.images || (editingListing.image ? [{ url: editingListing.image, path: null }] : []);
      setImages(imgs);
    }
  }, [editingListing]);
  ...
  const handleFiles = (files) => {
    const entries = Array.from(files).filter(f => f.type.startsWith("image/")).map(file => ({
      ...
      <div className="sticky top-0 z-30 border-b backdrop-blur-md"
        ...
        <div className="max-w-[860px] mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <motion.button whileHover={{ x: -3 }} whileTap={{ scale: 0.97 }} onClick={onCancel} disabled={saving}
          ...
          <motion.button
            ...
            className="flex items-center gap-1.5 h-9 px-5 text-white font-heading font-bold text-[11px] tracking-[0.1em] uppercase border-none cursor-pointer transition-colors duration-300 disabled:opacity-70"
            style={{ background: saved ? T.success : saving ? T.muted : T.primary }}
            onMouseEnter={e => { if (!saving && !saved) e.currentTarget.style.background = T.primaryHov; }}
      ...
      <div className="relative max-w-[860px] mx-auto px-4 sm:px-6 py-8 space-y-5" style={{ zIndex: 1 }}>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Field label="Category" required error={errors.category}>
              <TSelect value={form.category} onChange={e => up("category", e.target.value)}
                options={CATS} placeholder="Select category" error={errors.category} disabled={saving} />
            </Field>
          </div>
        ...
        <Card title="Description">
          <TTextarea value={form.description} onChange={e => up("description", e.target.value)}
            placeholder="Describe location highlights, access roads, power supply, finishing quality…" rows={5} disabled={saving} />
        </Card>

        {/* ── Images ── */}
        <Card title="Property Images" hint="First image becomes the cover">
          {/* Drop zone */}
          <div className="relative border-2 border-dashed transition-all duration-200 cursor-pointer"
            style={{ borderColor: T.border, background: "#faf9f7", minHeight: 90 }}
            onClick={() => !saving && document.getElementById("listing-img-upload").click()}
            onDragOver={e => { e.preventDefault(); e.currentTarget.style.borderColor = T.primary; e.currentTarget.style.background = T.primaryLt; }}
            onDragLeave={e => { e.currentTarget.style.borderColor = T.border; e.currentTarget.style.background = "#faf9f7"; }}
            onDrop={e => {
              e.preventDefault(); e.currentTarget.style.borderColor = T.border; e.currentTarget.style.background = "#faf9f7";
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 pointer-events-none">
              ...
              <p className="font-heading font-semibold text-[12px]" style={{ color: T.muted }}>Click or drag images to upload</p>
          ...
          <div className="flex gap-2">
            ...
            <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
              ...
              className="flex items-center gap-1 px-3 h-[46px] border font-heading font-bold text-[11px] uppercase cursor-pointer disabled:opacity-50 transition-colors"
                  ...
                  return (
                    <motion.div key={i} layout
                      ...
                      <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                        onClick={() => removeImg(i)} disabled={saving}
        ...
        <div className="flex justify-between items-center pt-2 flex-wrap gap-3">
          ...
          <motion.button
            ...
            onClick={handleSubmit} disabled={saving || saved}
            className="flex items-center gap-2 h-10 px-8 text-white font-heading font-bold text-xs tracking-[0.1em] uppercase border-none cursor-pointer transition-colors duration-300 disabled:opacity-70"
            style={{ background: saved ? T.success : saving ? T.muted : T.primary }}
            onMouseEnter={e => { if (!saving && !saved) e.currentTarget.style.background = T.primaryHov; }}
            onMouseLeave={e => { if (!saving && !saved) e.currentTarget.style.background = T.primary; }}>
...
function Card({ title, hint, children }) {
  return (
    <div className="border p-5 sm:p-6 space-y-4" style={{ borderColor: "#E5E0D8", background: "#FFFFFF" }}>
      <div className="flex items-center justify-between">
        <p className="font-heading font-bold text-[11px] tracking-[0.14em] uppercase" style={{ color: "#9F4325" }}>{title}</p>
        {hint && <span className="font-body text-[10px]" style={{ color: "#7A7A7A" }}>{hint}</span>}
      </div>
      {children}
    </div>
  );
