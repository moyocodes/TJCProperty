...
// Search moved from hero into the toolbar row (beside sort + view toggle).

import { useState, useMemo, useRef } from "react";
import { useNavigate } from "react-router-dom";
import { motion, AnimatePresence, useInView } from "framer-motion";
import {
  Search, X, LayoutGrid, List, MessageCircle, PhoneCall,
  ExternalLink, MapPin, Home, Filter,
} from "lucide-react";
...
function SearchBar({ value, onChange }) {
  ...
  return (
    <motion.div
      animate={{ width: focused || value ? 220 : 160 }}
      transition={{ duration: 0.25, ease: [0.22, 1, 0.36, 1] }}
      className={`relative flex items-center border bg-white transition-colors ${focused ? "border-primary-600" : "border-neutral-200"}`}
    >
      <Search size={13} className="absolute left-3 text-neutral-400 pointer-events-none flex-shrink-0" />
      <input
        ref={inputRef}
        value={value}
        ...
        className="w-full h-9 pl-9 pr-8 font-body text-[12px] text-secondary-600 placeholder:text-neutral-400 bg-transparent border-none outline-none"
      />
      <AnimatePresence>
        {value && (
          <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
            ...
            className="absolute right-2 w-4 h-4 flex items-center justify-center bg-transparent border-none cursor-pointer text-neutral-400 hover:text-secondary-600">
            <X size={10} />
          </motion.button>
        )}
      </AnimatePresence>
      ...
      <div className="p-5">
        ...
        <div className="flex justify-between items-center pt-3 border-t border-neutral-100 gap-2">
          <span className="font-body text-[11px] text-neutral-400">
            <strong className="font-heading text-[14px] text-primary-600">{listing.units}</strong>{" "}
...
function NativeSelect({ value, onChange, options }) {
  return (
    <select value={value} onChange={e => onChange(e.target.value)}
      className="appearance-none bg-white border border-neutral-200 px-4 py-2.5 pr-9 font-heading font-semibold text-[12px] text-secondary-600 cursor-pointer outline-none focus:border-primary-600 transition-colors h-9"
      style={{
...
export default function ListingsPage() {
  ...
  const activeFilters = [
    typeFilter !== "all", catFilter !== "all", priceFilter !== 0,
  ...
  const clearAll = () => {
    setSearch(""); setTypeFilter("all"); setCatFilter("all"); setPriceFilter(0); setSort("newest");
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-100">
      ...
      <div className="relative bg-secondary-600 overflow-hidden">
        ...
        <div className="relative max-w-[1200px] mx-auto px-[5%] pt-28 pb-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <h1 className="font-heading font-bold text-white text-3xl leading-[1.1]">
      ...
      <div className="max-w-[1200px] mx-auto px-[5%] py-10">
        <div className="flex gap-8">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* ── Toolbar: mobile filter + result count + chips | search + sort + view ── */}
            <div className="mb-6 space-y-3">

              {/* Row 1: mobile filter btn + result count + active chips */}
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <p className="font-body text-[13px] text-neutral-500">
                  <strong className="font-heading text-secondary-600">{results.length}</strong>{" "}
              ...
              {/* Row 2: search (left) | sort + view toggle (right) */}
            ...
            {!loading && results.length === 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <p className="font-body text-sm mt-1 text-neutral-500 max-w-[280px] mx-auto">
        ...
        <FadeUp className="mt-16">
          <div className="relative overflow-hidden bg-secondary-600 p-8 sm:p-12">
            ...
            <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div>
                <p className="font-heading font-bold text-[11px] tracking-[0.18em] uppercase text-primary-400 mb-2">
